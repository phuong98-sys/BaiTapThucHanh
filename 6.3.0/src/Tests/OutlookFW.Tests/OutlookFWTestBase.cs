...
using OutlookFW.Migrations.SeedData;
using OutlookFW.MultiTenancy;
using OutlookFW.Users;
using Castle.MicroKernel.Registration;
using Effort;
using EntityFramework.DynamicFilters;

namespace OutlookFW.Tests
{
    public abstract class OutlookFWTestBase : AbpIntegratedTestBase<OutlookFWTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected OutlookFWTestBase()
        {
        ...
        protected void UsingDbContext(Action<OutlookFWDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<OutlookFWDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<OutlookFWDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<OutlookFWDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<OutlookFWDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OutlookFWDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<OutlookFWDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OutlookFWDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<OutlookFWDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<OutlookFWDbContext, Task<T>> func)
        {
            T result;
