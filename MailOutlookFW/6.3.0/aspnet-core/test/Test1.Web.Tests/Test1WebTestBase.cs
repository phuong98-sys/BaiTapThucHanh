...
{
    public abstract class Test1WebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static Test1WebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(Test1WebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<Test1DbContext> action)
        {
            using (var context = IocManager.Resolve<Test1DbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<Test1DbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<Test1DbContext, Task> action)
        {
            using (var context = IocManager.Resolve<Test1DbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<Test1DbContext, Task<T>> func)
        {
            T result;
